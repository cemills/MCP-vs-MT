...
ends_area = 2 * np.pi * radius_mcp**2 # [=] m2
...
tube_model = MTubes(fit_fun, Time.iloc[-1], mcp_surface_area, mcp_volume,
...
time_concat, sol_concat = tube_model.generate_time_series(init_conds, params)
...
mcp_masses_org = init_conds_list[:5] * mcp_volume * params["nmcps"] * tube_model.optical_density_ts(Time.iloc[-1])\
                 * OD_TO_COUNT_CONC * external_volume
cell_masses_org = init_conds_list[5:10] * cell_volume * tube_model.optical_density_ts(Time.iloc[-1])* OD_TO_COUNT_CONC\
                  * external_volume
ext_masses_org = init_conds_list[10:] * external_volume

mcp_masses_fin = sol_concat[-1,:5] * mcp_volume * params["nmcps"] *tube_model.optical_density_ts(Time.iloc[-1]) \
                 * OD_TO_COUNT_CONC * external_volume
cell_masses_fin = sol_concat[-1,5:10] * cell_volume * tube_model.optical_density_ts(Time.iloc[-1]) * OD_TO_COUNT_CONC \
                  * external_volume
...
np.savetxt("dN_constSA_constNenz_tepMCP_t.csv",time_concat,delimiter=",")
np.savetxt("dN_constSA_constNenz_tepMCP.csv",sol_concat,delimiter=",")